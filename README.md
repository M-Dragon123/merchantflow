...
面向中小商家的多端订单与库存管理系统。整体方案、数据库设计、接口/页面清单与开发里程碑见 [docs/PLAN.md](docs/PLAN.md)；**部署步骤见 [DEPLOY.md](DEPLOY.md)**；**常用命令与本地/Docker 切换见 [docs/COMMANDS.md](docs/COMMANDS.md)**。当前进度：M1–M8 全部完成（工程骨架、JWT 认证、商品与库存、订单状态机、工作台与报表、移动端仓库模式、管理后台、PWA 与部署收尾）；阶段 2「AI 运营助手」为预留模块。
